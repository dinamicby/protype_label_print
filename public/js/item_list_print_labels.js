(() => {
  ...
  // гарантируем, что объект существует и не перетрем другие настройки
  frappe.listview_settings["Item"] = Object.assign(
    {},
    frappe.listview_settings["Item"],
    {
      onload(listview) {
        // сначала выполняем прежний onload ERPNext (если был)
        if (typeof prev === "function") {
          try {
            prev(listview);
          } catch (e) {
            console.warn("Prev onload error:", e);
          }
        }

        // затем добавляем наш пункт Actions
        listview.page.add_actions_menu_item(__("Print Labels"), async () => {
          ...
          const d = new frappe.ui.Dialog({
            ...
            fields: [
              ...
              {
                fieldname: "show_position",
                label: __("Позиция на складе (Item.storage_position)"),
                fieldtype: "Check",
                default: 1,
              },
              {
                fieldname: "show_qr",
                label: __("QR с ссылкой на товар"),
                fieldtype: "Check",
                default: 1,
              },
              {
                fieldname: "qr_url_base",
                label: __(
                  "Base URL (например https://erp.company.com/app/item)"
                ),
                fieldtype: "Data",
                depends_on: "eval:doc.show_qr",
                default: "",
              },
              ...
              {
                fieldname: "h_gap_mm",
                label: __("Гор. зазор мм"),
                fieldtype: "Int",
                default: 4,
              },
              {
                fieldname: "v_gap_mm",
                label: __("Вер. зазор мм"),
                fieldtype: "Int",
                default: 2,
              },
              {
                fieldname: "page_margins_mm",
                label: __("Поля страницы мм"),
                fieldtype: "Int",
                default: 6,
              },
              ...
              {
                fieldname: "per_item_qty",
                label: __("Штук на товар"),
                fieldtype: "Int",
                default: 1,
                reqd: 1,
              },
        ...
        // подсказочный лог, чтобы убедиться, что файл точно подхватился
        console.debug("[protype_label_print] Item List onload patched");
      },
    }
  );
})();
